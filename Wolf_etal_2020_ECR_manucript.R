...
dat_reduce <- dat %>%
  select(c(Num_publications_first, Num_publications_last,
           ...
           gender_first, gender_last, ECR_status_first_simplified, ECR_status_last_simplified))
...
# T-test to check if there is a significant difference in preprints between ECR status of first and last authors
t.test(dat_reduce$Num_preprints_first~dat_reduce$ECR_status_first_simplified)
t.test(dat_reduce$Num_preprints_last~dat_reduce$ECR_status_last_simplified)
...
ggbox_1b <- ggplot(data=subset(dat_reduce,!is.na(gender_first)),
...
#Figure 1b
ggbox_1a <- ggplot(data=subset(dat_reduce,!is.na(gender_last)),
...
#Figure 1c
ggbox_1c <- ggplot(data=subset(dat_reduce,!is.na(gender_first)),
...
#Figure 1d
ggbox_1d <- ggplot(data=subset(dat_reduce,!is.na(gender_last)),
